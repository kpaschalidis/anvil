...
- [ ] Quick profile: sanitize snippets (“Why” is readable, no nav/relative links)
- [ ] Quick profile: persist `research/synthesis_input.json` (single debugging artifact)
- [ ] Quick profile: improve synthesis prompt to target 8 citations / 3 domains (warn-only)
- [ ] Quick profile: post-synthesis validation + one repair attempt (hard-fail grounding only)
- [ ] Quick profile: report quality diagnostics line (good/limited)
